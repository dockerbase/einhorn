...
run:
	docker run -it --rm --name dockerbase-einhorn $(NAME):$(VERSION)
...
version:
	...
	docker run -it --rm $(NAME):$(VERSION) bash -lc " rvm -v ; ruby -v ; rails --version " | tee -a COMPONENTS
	docker run -it --rm $(NAME):$(VERSION) bash -lc " echo -n einhorn: ; einhorn --version " | tee -a COMPONENTS
	dos2unix COMPONENTS
